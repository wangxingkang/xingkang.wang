...
收录 Rust 开发库与生态工具。成品软件可按用途查找[AI 编程工具](/awesome/vibe-coding)、[编辑器](/awesome/ide)、[终端工具](/awesome/ops#终端)和[桌面应用](/awesome/desktop#应用与工具)。
...
数据库与搜索引擎见[运维 / 数据库与搜索](/awesome/ops#数据库与搜索)。
...
通用的版本管理、Git 与发布工具见[工程化](/awesome/engineering)。
